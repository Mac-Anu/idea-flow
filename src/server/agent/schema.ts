...
export const explainRequestSchema = z
    .object({
        text: z
            .string()
            .min(1, "需要解释的文本不能为空")
            .max(2000, "文本太长啦，请分段划词解释哦 (限制2000字内)")
            .meta({ description: "用户划词选中的文本" }),
        context: z
            .string()
            .optional()
            .meta({ description: "选中文本所在的上下文段落，帮助AI更准确理解" }),
    })
...
export const editorRequestSchema = z
    .object({
        text: z
            .string()
            .max(2000, "选中的文本太长啦 (限制2000字内)")
            .meta({ description: "需要AI处理的文本内容" }),
...
export const reviewSchema = z
    .object({
        passed: z.boolean().meta({ description: "回答是否完全合格,无需再修改时为true" }),
        reason: z.string().meta({ description: "若不合格,说明具体问题以便重写;合格则填 OK" }),
    })
    .meta({ description: "反思审查节点的结构化评审结论" });
